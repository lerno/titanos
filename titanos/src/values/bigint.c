...
bool bigint_fits_in_bits(const BigInt *big_int, size_t bit_count, bool is_signed)
{
    ...
    if (bit_count == 0)
    {
        return bigint_cmp_zero(big_int) == INT_EQ;
    }
    if (big_int->digit_count == 0)
    {
        return true;
    ...
    IntCmp min_cmp = bigint_cmp(big_int, &min_value);
    IntCmp max_cmp = bigint_cmp(big_int, &max_value);

    return (min_cmp == INT_GT || min_cmp == INT_EQ) && (max_cmp == INT_LT || max_cmp == INT_EQ);
}
...
void bigint_div_floor(BigInt *dest, const BigInt *op1, const BigInt *op2)
{
    if (op1->is_negative != op2->is_negative)
    {
        ...
        if (bigint_cmp(&mult_again, op1) != INT_EQ)
        {
            BigInt tmp = { 0 };
            bigint_init_bigint(&tmp, dest);
            BigInt neg_one = { 0 };
...
void bigint_shr(BigInt *dest, const BigInt *op1, const BigInt *op2)
{
    ...
    const uint64_t *op1_digits = bigint_ptr(op1);
    uint64_t
            shift_amt = bigint_as_unsigned(op2);
...
IntCmp bigint_cmp(const BigInt *op1, const BigInt *op2)
{
    ...
    {
        return op1->is_negative ? INT_LT : INT_GT;
    }
    else if (op2->digit_count > op1->digit_count)
    {
        return op1->is_negative ? INT_GT : INT_LT;
    }
...
IntCmp bigint_cmp_zero(const BigInt *op)
{
    ...
    return op->is_negative ? INT_LT : INT_GT;
}
